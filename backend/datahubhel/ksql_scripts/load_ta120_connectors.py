...
SINK_CONFIGURATIONS = {
    "observation-sink": {
        ...
        "topics.regex": r"fi\.fvh\.observations\.\w+\.\w+",
        "table.name.format": settings.SINK_TABLE_NAME,
        ...
        "connection.user": settings.SINK_DB_USERNAME,
        "connection.password": settings.SINK_DB_PASSWORD,
        "auto.create": "true",
        "auto.evolve": "true",
        "insert.mode": "upsert",
...
def _load_connector(configuration_name):
    ...
    response = requests.post(
        f"{settings.KAFKA_CONNECT_URL}/connectors", headers=headers, json=data
    )
    check_errors(response)
